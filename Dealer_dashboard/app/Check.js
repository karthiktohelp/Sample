...
class Check extends Component {
	...
	constructor(props) {
		super(props);
		console.log("Check Internet connection ", navigator.onLine);
	...
	componentDidMount() {
		...
		axios
			.get(
			URL.ROOT_URL + '/audit/' + this.props.id
			)
			.then(response => {
				...
				for (var j = 0; j < rr.audit_worksheets.length; j++) {
					if (rr.audit_worksheets[j].contract !== undefined) {
						...
						if (rr.audit_worksheets[j].contract.worksheet.selection_vehicle_value !== null) {
						...
						if (rr.audit_worksheets[j].contract.worksheet.doc_fee.overcharge !== null) {
						...
						if (rr.audit_worksheets[j].contract.worksheet.debt_cancellation !== null) {
				...
				if (mybuyers_guides_missing_arrayString !== '') {				   //   23
				    ...
				    voilationlist.push(mybuyers_guides_missing_violation);
				}
				if (rr.inventory_worksheet !== undefined) {
					if (rr.inventory_worksheet.buyers_guides.spanish_bg.none_present === true) {      // 24
					...
					if (rr.inventory_worksheet.advertising.violation.front_license_plate_tile.disclosures_in_deal_jackets === false) {      // 45
				...
				if (mywas_is_former_price_comparison_violationarrayString !== '') {      									// 44
				        ...
				        voilationlist.push(mywas_is_former_price_comparison_violationarrayStringviolation);
				}
				...
				this.setState({ list_of_audits: gg, violationn: voilationlist });
			})
			.catch(e => {
				console.log("Error", e);
			});
	...
	render() {
		...
		return (
			<div className="change">
				...
				<h3>{this.props.id}</h3>
				<br />
				<div style={styles.wording_left}>
					{this.state.list_of_audits.map((row, index) =>
						<div key={0} >Dealer name is : {row.dealer_info.dealer_name}</div>
					)}
					{this.state.list_of_audits.map((row, index) =>
						<div key={0}>Auditor name is : {row.audit_info.auditor_id}</div>
					)}
				</div>
				<div style={styles.wording_right}>
					{this.state.list_of_audits.map((row, index) =>
						<div key={0} >Dealer id : {row.dealer_id}</div>
					)}
					{this.state.list_of_audits.map((row, index) =>
						<div key={0} >Audit date is : {row.audit_info.audit_date}</div>
					)}
				</div>

				<div style={styles.wording_left}>
					{this.state.violationn.map((row, index) =>
						<div key={index} ><li> {row}</li></div>
					)}
				</div>
				...
				{/*{this.state.list_of_audits.map((row, index) =>  row.audit_worksheets !== null ? <div>have data</div>   : <div>no complete data</div>  
                    )} */}

				{/*<br/> */}
				{/*<div style={styles.wording_left}>*/}
				{/*lease audit worksheet*/}
				{/*{this.state.list_of_audits.map((row, index) => row.audit_worksheets.map((raw,indix) => raw.lease !== null ? raw.lease.e_lease.lease_signed_copy == false ? <div><li>Copies of the signed electronic copy of the electronic lease agreement must be retained in every sales jacket in which such agreements were executed.</li></div> : <div>its null</div> : <div>its null</div> ) 
                    )} */}
				{/*lease audit worksheet*/}
				{/*dmv review bullet points*/}
				{/*{this.state.list_of_audits.map((row, index) =>  row.dmv_Review.is_refund_checks == null ? <div><li>The dealership is not refunding its overcharges in a timely manner.</li></div>   : <div></div>  
                    )} 
                {this.state.list_of_audits.map((row, index) =>  row.dmv_Review.report_of_sale == false ? <div><li>Voided Reports of Sale fail to have a statement of facts attached as required by the Department of Motor Vehicles and processes must be created to ensure compliance with this requirement.</li><Button activeStyle={{position:'relative', top: 1}}>No law</Button></div>   : <div></div>  
                    )} 
                </div>         */}
				{/*dmv review bullet points*/}
				{/*
                {this.state.list_of_audits.map((row, index) =>  row.audit_worksheets !== null ? row.audit_worksheets.map((raw,indix) =>{
                    <div key={indix}> here {raw.stk_no}</div>}) : <div>no complete data</div>  
                    )}        */}
			</div>
		);
	}
